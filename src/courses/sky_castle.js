...
(function () {
  ...
  // ※コース形状修正(立体交差化・ヘアピン拡大)で弧長が変わったため、
  //   物理アンカー実測に基づき全t値を-0.017補正済み
  const GAP_ZONE = {
    jump1: { t0: 0.3053, t1: 0.3071 },
    gap1: { t0: 0.3097, t1: 0.3167 },   // jump1始端+13u以内 / jump1終端+3u後
    jump2: { t0: 0.3591, t1: 0.3609 },
    gap2: { t0: 0.3635, t1: 0.3705 },   // jump2始端+13u以内 / jump2終端+3u後
  };
  ...
  const FALL_ZONES = [
    { t0: 0.145, t1: 0.383 },   // 高架の登り〜螺旋〜ギャップ地帯(形状修正に合わせ前倒し)
    { t0: 0.413, t1: 0.583 },   // 急降下ストレート
  ];
  ...
  const P6 = {
    ...
    bannerSpots: [
      ...
      { t: 0.46, side: -1, text: 'ASTRO GEAR' },
      { t: 0.94, side: 1, text: 'STAR CITADEL CUP' },
    ],
  ...
  Game.courses.skyCastle = {
    ...
    controlPoints: [
      ...
      { x: 3, y: 0, z: -26, w: 9.0 },       // 1 (ターンへ滑らかに導入)
      { x: 30, y: 1, z: 4, w: 7.5 },        // 2 連続コーナー(回転半径を確保するため外へ)
      { x: 44, y: 1.8, z: 16, w: 7.0 },     // 3
      { x: 31, y: 4.9, z: -11, w: 7.0 },    // 4
      { x: 12, y: 9.7, z: -17, w: 6.8 },    // 5 螺旋登り入口(スタート直線を跨ぐ立体交差、クリアランス6以上)
      { x: 27, y: 11.3, z: -27, w: 6.4 },   // 6
      ...
      { x: -26, y: 7.8, z: -310, w: 8.8 },  // 18 (リターン直線x=0との並走を避け西へ分離)
      { x: -24, y: 1.2, z: -352, w: 9.0 },  // 19 着地(リターン直線と横に24離して並走)
      { x: -2, y: 0, z: -392, w: 9.0 },     // 20 広いヘアピンの頂点→スタートへの戻り直線
    ],
